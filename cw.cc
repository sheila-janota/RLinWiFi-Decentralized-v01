...
std::vector<uint64_t> g_rxPktNum(n_node, 0);
std::vector<uint64_t> g_txPktNum(n_node, 0);
...
std::string MyGetExtraInfo(void)
{
    ...
    for (int sta_id = 0; sta_id < nWifi; sta_id++)
    {
    ...
    if (verbose)
        ...
        std::cout << "MyGetExtraInfo:" << myInfo << endl;
    return myInfo;
}
...
bool MyExecuteActions(Ptr<OpenGymDataContainer> action)
{
    ...
    if (type == "discrete")
    {
        ...
        std::cout << "MyExecuteActions-CW: " << CW << std::endl;
    }
...
float MyGetReward(void)
{
    ...
    static uint32_t last_packets[1];
    //static float last_reward = 0.0;
    static float last_station_reward[1];
...
Ptr<OpenGymDataContainer> MyGetObservation()
{
    ...
    //std::vector<uint32_t> shape(2, 300);
...
void recordHistory()
{
    ...
    static std::vector<uint64_t> last_rx(n_node, 0);
    static std::vector<uint64_t> last_tx(n_node, 0);
